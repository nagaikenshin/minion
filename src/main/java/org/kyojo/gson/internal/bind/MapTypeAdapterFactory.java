 ...
 * <pre>org.kyojo.gson.JsonParseException: Expecting object found: "(5,6)"
 *   at org.kyojo.gson.JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at org.kyojo.gson.ObjectNavigator.navigateClassFields
 *   ...</pre>
 *
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    @Override public void write(JsonWriter out, Map<K, V> map) throws IOException {
      ...
      if (!complexMapKeySerialization) {
        ...
        List<Map.Entry<K, String>> kKeys = new LinkedList<Map.Entry<K, String>>(kStrs.entrySet());
        ...
        out.beginObject();
        for (Map.Entry<K, String> entry : kKeys) {
          out.name(String.valueOf(entry.getValue()));
          valueTypeAdapter.write(out, map.get(entry.getKey()));
        }
        out.endObject();
        return;
      }
      ...
      List<JsonElement> jeKeys = new ArrayList<JsonElement>(map.size());
      ...
      if (hasComplexKeys) {
        out.beginArray();
        for (int i = 0, size = jeKeys.size(); i < size; i++) {
          out.beginArray(); // entry array
          Streams.write(jeKeys.get(i), out);
        ...
        out.beginObject();
        for (int i = 0, size = jeKeys.size(); i < size; i++) {
          JsonElement keyElement = jeKeys.get(i);
